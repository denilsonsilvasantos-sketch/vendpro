...
export default function Banner({ banners }: { banners?: BannerData[] }) {
  ...
  return (
    <div 
      className="relative w-full h-[300px] md:h-[400px] overflow-hidden bg-[#1a0a2e] group"
    ...
    >
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={`absolute inset-0 flex items-center ${slides[current].className}`}
        >
          {/* Decorative Circles */}
          <div className="absolute w-[340px] h-[340px] bg-white/10 rounded-full -right-[60px] -top-[80px]" />
          <div className="absolute w-[200px] h-[200px] bg-primary-light/10 rounded-full right-[120px] -bottom-[60px]" />
          ...
          <div className="relative z-10 px-8 md:px-16 max-w-2xl">
            ...
            >
              {slides[current].title.split('\n').map((line, i) => (
                <span key={i} className={i === 1 ? 'text-primary-light' : ''}>
                  {line}{i === 0 && <br />}
                </span>
              ))}
            </motion.h2>
            ...
            >
              <button className="bg-white text-primary px-7 py-3 rounded-full text-sm font-bold shadow-lg shadow-primary/30 hover:-translate-y-0.5 transition-all">
                {slides[current].cta} →
              </button>
              <button className="hidden md:flex bg-transparent text-white/85 border border-white/35 px-6 py-2.5 rounded-full text-sm font-medium hover:bg-white/10 transition-all">
                Saiba mais
              </button>
            </motion.div>
          </div>
          ...
          <div className="hidden lg:flex absolute right-20 top-1/2 -translate-y-1/2 gap-4 items-end z-10">
            {slides[current].visuals.map((v, i) => (
              <motion.div
                key={i}
